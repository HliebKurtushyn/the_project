...
def get_cart_items_with_stock_check(request, cart):
    all_items = get_items(request, cart)
    cart_items = [item for item in all_items if item.product.stock > 0]

    if not cart_items:
        if all_items:
            messages.warning(request, "Товари у вашому кошику закінчилися на складі.")
        else:
...
def checkout(request):
    ...
    cart_items = get_cart_items_with_stock_check(request, cart)

    if isinstance(cart_items, HttpResponseRedirect):
        return cart_items

    return render(
        request,
        "checkout/checkout_detail.html",
...
def orders_history(request):
    ...
    if user:
        orders_history_queryset = OrdersHistory.objects.filter(user=user).order_by(
            "-id"
        )
        ...
        return render(request, "checkout/orders_history.html", {"orders": orders_list})
    ...
    for session_key, session_value in request.session.items():
        if (
            ...
            and session_key.startswith("order ")
